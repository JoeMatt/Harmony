...
class FetchRemoteRecordsOperation: Operation<(Set<RemoteRecord>, Data), FetchError> {
    ...
    init(changeToken: Data?, coordinator: SyncCoordinator, recordController _: RecordController) {
    ...
    override func main() {
                    ...
                    do {
                        ...
                        if UserDefaults.standard.isDebugModeEnabled {
                            for record in updatedRecords {
                                os_log("Fetched RecordID: %@. Hash: .", type: .info, String(describing: record.recordID), record.sha1Hash)
                            }
                        }
        ...
        if let changeToken = changeToken {
            let operation = ServiceOperation(coordinator: coordinator) { completionHandler -> Progress? in
                    self.service.fetchChangedRemoteRecords(changeToken: changeToken, context: context, completionHandler: completionHandler)
                }
            ...
            let operation = ServiceOperation(coordinator: coordinator) { completionHandler -> Progress? in
                    self.service.fetchAllRemoteRecords(context: context, completionHandler: completionHandler)
                }
            operation.resultHandler = { result in
                do {
        ...
        progress.addChild(fetchOperation.progress, withPendingUnitCount: progress.totalUnitCount)
